...
void FftObject::plot(vector<float>& buffer, const ofRectangle &r, bool bDrawLogScale) {
    ...
    for (int i = 0; i < n; i++) {
		...
		ofDrawRectangle(v.x, offset, xinc, -(offset - v.y));
		//cout << "Drawing log?";
        }else{
            m.addVertex(ofVec3f(i*xinc+r.x, offset - buffer[i]*r.height ));
		ofDrawRectangle(i*xinc+r.x, offset, xinc, offset - buffer[i]*r.height);
        }
...
void FftObject::getAmpAtFreq() {
	int freq = ofMap(ofGetMouseX(), 0, ofGetWidth(), 0, 22050);
